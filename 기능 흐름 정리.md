...
.. 분기: 병렬적 로직에서 "대화 모드"는 바로 진입, 행동 모드는 하단 분기로 재구성  
<br>
...
인풋은 기본적으로: { message: 받아온 raw_text, contextState: Spring에서 조회한 맥락 }.. 으로 
"대화 모드"에서 "행동 모드" 전환은 명시적 trigger가 되도록 로직 고도화 필요 있음    
<br>  

**사용자 → LLM 까지의 요청 흐름 정리**  
프론트 입력 → Build(controller → chatService).. Redis 캐시 조회  
→ AiChatEngine(LLMrequired → getLLMresponse)  
→ AiChatClient(generateResponse → restClient.post: 실제 접속)  
.. Redis 캐시 저장  

*왜 캐시 저장을 수행하는가?*.. 룰 베이스 기반이 먼저 진입: redis에서 key 찾아 불러오기 可  
